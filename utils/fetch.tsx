class FetchWrapper {
    ...
    async request<T>({ endpoint, options = {}, query }: RequestFunctionParams): Promise<T> {
      const url = this.constructURL(endpoint, query);
      const response = await this.fetchData(url, options);
      return this.handleResponse<T>(response);
    }
  
    private constructURL(endpoint: string, query?: Record<string, string | number | boolean>): string {
